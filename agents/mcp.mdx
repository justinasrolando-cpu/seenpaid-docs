---
title: "MCP server"
description: "Connect Claude, ChatGPT, Hermes, Cursor or any MCP client to seenpaid in one step."
---
...
<Tabs>
  ...
  <Tab title="Hermes (YAML)">
    [Hermes](https://github.com/NousResearch/hermes) speaks MCP natively. Add seenpaid to `~/.hermes/config.yaml` and it can post and read revenue from any chat (Telegram, Discord, Slack…):
    ...
    Restart Hermes, then message it — *"post my launch thread everywhere at 9am and tell me which one earns."* Prefer a browser consent flow? Use `auth: oauth` instead of the `headers` block.
  </Tab>
